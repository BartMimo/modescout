    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        supabase.from('profiles').select('role').eq('id', user.id).single()
          .then(({ data }) => setUser({ email: user.email!, role: data?.role }))
      }
    })
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_, session) => {
      if (session?.user) {
        supabase.from('profiles').select('role').eq('id', session.user.id).single()
          .then(({ data }) => setUser({ email: session.user.email!, role: data?.role }))
      } else {
        setUser(null)
      }
    })
    ...
    <header className="sticky top-0 z-50 bg-[#0D0D0D] text-[#F5F5EF]">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-1 w-48 bg-[#0D0D0D] border border-[#333] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all text-sm">
                <div className="px-4 py-2 text-[#999] text-xs border-b border-[#333] truncate">{user.email}</div>
          ...
          {/* Hamburger */}
          <button
            className="md:hidden p-2 hover:text-[#CDFF00] transition-colors"
      ...
      {open && (
        <nav className="md:hidden border-t border-[#222] px-4 py-4 flex flex-col gap-4 text-sm">
          ...
          {!user && <Link href="/inloggen" onClick={() => setOpen(false)} className="hover:text-[#CDFF00]">Inloggen / Registreren</Link>}
          {user && <Link href="/bestellingen" onClick={() => setOpen(false)} className="hover:text-[#CDFF00]">Mijn bestellingen</Link>}
          {(user?.role === 'brand' || user?.role === 'admin') && (
